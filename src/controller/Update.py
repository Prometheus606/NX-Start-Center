...
def check_for_update(model, repo_url):
    try:
        ...
        if response.status_code == 200:
            ...
            if latest_version.lower() > model.app_version.lower():
                ...
                if user_accepts_update(model.app_version, latest_version, description, title):
                    if downloadet_update(model, download_url, latest_version):
                        messagebox.showinfo("Update Heruntergeladen", f"Das Update wurde erfolgreich heruntergeladen und liegt nun im downloads verzeichnis. Bitte installiere nun das Update.")
                        return True
        else:
...
def downloadet_update(model, download_url, latest_version):
